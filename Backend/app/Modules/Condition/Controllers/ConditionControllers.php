...
class ConditionControllers extends Controller {
    ...
    {   
        if($request->ajax()){
            $data = Condition::dataList();
            return Datatables::of($data['data'])->make(true);
        }
        return view('Condition.Views.index');
    }
    ...
    public function edit($id) {
        ...
        $menuObj = Condition::NotDeleted()->find($id);
    ...
    public function update($id) {
        ...
        WebActions::newType(2,'Condition');
    ...
    public function create() {
        ...
        WebActions::newType(1,'Condition');
    ...
    public function delete($id) {
        $id = (int) $id;
        $menuObj = Condition::getOne($id);
        WebActions::newType(3,'Condition');
    ...
    public function fastEdit() {
        ...
        foreach ($input['data'] as $item) {
            $col = $item[1];
            $menuObj = Condition::NotDeleted()->find($item[0]);
            $menuObj->$col = $item[2];
            $menuObj->updated_at = DATE_TIME;
            $menuObj->updated_by = USER_ID;
            $menuObj->save();
        ...
        WebActions::newType(4,'Condition');
        return \TraitsFunc::SuccessResponse('تم التعديل بنجاح');
    }

    public function arrange() {
        $data = Condition::dataList();
        return view('Condition.Views.arrange')->with('data', (Object) $data);;
    }
    ...
    public function sort(){
        ...
        for ($i = 0; $i < count($ids) ; $i++) {
            Condition::where('id',$ids[$i])->update(['sort'=>$sorts[$i]]);
        }
    ...
    public function charts() {
        ...
        $addCount = WebActions::getByDate($date,$start,$end,1,'Condition')['count'];
        $editCount = WebActions::getByDate($date,$start,$end,2,'Condition')['count'];
        $deleteCount = WebActions::getByDate($date,$start,$end,3,'Condition')['count'];
        $fastEditCount = WebActions::getByDate($date,$start,$end,4,'Condition')['count'];

        $data['chartData1'] = $this->getChartData($start,$end,1,'Condition');
        $data['chartData2'] = $this->getChartData($start,$end,2,'Condition');
        $data['chartData3'] = $this->getChartData($start,$end,4,'Condition');
        $data['chartData4'] = $this->getChartData($start,$end,3,'Condition');
        $data['counts'] = [$addCount , $editCount , $deleteCount , $fastEditCount];
        $data['title'] = 'شروط العضوية';
        $data['miniTitle'] = 'شروط العضوية';
        $data['url'] = 'conditions';
